...
- **Sync** — Concurrent paginated pull of GitHub starred repos into local SQLite (preserves AI analysis on re-sync). Supports periodic auto-sync mode.
...
- **Release watermark** — Subscribed repos track the latest fetched release timestamp, so only new releases are retrieved on each pull.
